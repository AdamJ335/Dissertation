...
public class WaypointTest : MonoBehaviour
{
    ...
    float accel = 1; //This is the rate of accelleration after the void "Accell()" is called. Higher values will cause the object to reach the "speedLimit" in less time.

    float inertia = 1; //This is the the amount of velocity retained after the void "Slow()" is called. Lower values cause quicker stops. A value of "1.0" will never stop. Values above "1.0" will speed up.

    float speedLimit = 10; //This is as fast the object is allowed to go.

    float minSpeed = 1; //This is the speed that tells the functon "Slow()" when to stop moving the object.

    float stopTime = 1; //This is how long to pause inside "Slow()" before activating the void "Accell()" to start the script again.

    //This variable "currentSpeed" is the major player for dealing with velocity.
    //The "currentSpeed" is mutiplied by the variable "accel" to speed up inside the void "accell()".
    //Again, The "currentSpeed" is multiplied by the variable "inertia" to slow things down inside the void "Slow()".
    private float currentSpeed = 0;

    //The variable "voidState" controlls which void, "Accell()" or "Slow()", is active. "0" is void "Accell()" and "1" is void "Slow()".
    private int voidState = 0;

    //The next two variables are used to make sure that while the void "Accell()" is running, the void "Slow()" can not run (as well as the reverse).
    ...
    //voids! They do all the work.
    //You can use the built in voids found here: [url]http://unity3d.com/support/documentation/ScriptReference/MonoBehaviour.html[/url]
    //Or you can declare your own! The void "Accell()" is one I declared.
    //You will want to declare your own voids because theres just certain things that wont work in "Update()". Things like Coroutines: [url]http://unity3d.com/support/documentation/ScriptReference/index.Coroutines_26_Yield.html[/url]


    //The void "Start()" is called just before anything else but only one time.
    void Start()
    {
        voidState = 0; //When the script starts set "0" or void Accell() to be active.
    ...
    //The void "Update()" is called every frame. It can get slow if overused.
    void Update()
    {
        if (voidState == 0) //If voidState variable is currently "0" then run "Accell()". Withouth the "if", "Accell()" would run every frame.
        {
            Accell();
        }
        if (voidState == 1) //If voidState variable is currently "1" then run "Slow()". Withouth the "if", "Slow()" would run every frame.
        {
            Slow();
        }
    ...
    //The void "OnTriggerEnter" is called when a collision happens.
    void OnTriggerEnter()
    {
        voidState = 1; //When the GameObject collides with the waypoint's collider, activate "Slow()" by setting "voidState" to "1".
    ...
    IEnumerator Slow()
    {
        ...
        {
            currentSpeed = 0; // ... Stop the movement by setting "currentSpeed to Zero.
            yield return new WaitForSeconds(stopTime); //Wait for the amount of time set in "stopTime" before moving to next waypoint.
            voidState = 0; //Activate the void "Accell()" to move to next waypoint.
        }
    }
}
